...
class KePaymentController extends Controller
{    
	...
	$nidcccc=0;
	  ...
	  if(array_key_exists("GETCIN_RESULT",$input))
	  {
		...
		$input['GRN'] = $dbl_verification[2];//Setting GRN value
		$input['DEPARTMENT_ID'] = $dbl_verification[12];//Setting DEPARTMENT_ID value
		$input['STATUS'] = $dbl_verification[20];//Setting status value
		$input['AMOUNT'] = $dbl_verification[4];//Setting AMOUNT value
		$input['CHALLANTIMESTAMP'] = $dbl_verification[14];//Setting status value
		$input['BANKTIMESTAMP'] = $dbl_verification[16];//Setting CHALLANTIMESTAMP value
		$input['BANK_CODE'] = $dbl_verification[18];//Setting BANK_CODE value
		$input['PRN'] = $dbl_verification[10];//Setting PRN value
		$input['BANKREFNO'] = "";//Setting BANKREFNO value
		$input['REMARKS'] = "";//Setting REMARKS value
		$input['PARTYNAME'] = "";//Setting PARTYNAME value
		$input['BANKCIN'] = "";//Setting BANKCIN value
	  }
	if(isset($input['GRN']) && isset($input['DEPARTMENT_ID'])){
		 try {
			...
			if($input['CHALLANTIMESTAMP']){
				$challan_timestamp = date('Y-m-d H:i:s',strtotime($input['CHALLANTIMESTAMP']));
			}else{
				$challan_timestamp = '';
			}
			if($input['BANKTIMESTAMP']){
				$bank_timestamp = date('Y-m-d H:i:s',strtotime($input['BANKTIMESTAMP']));
			}else{
				$bank_timestamp = '';
			}
			$paymentconfirmationnumber = ($input["BANKCIN"]<>'')?$input["BANKCIN"]:$chkd[0]->payment_confirmation_number_cin;
			$bank_code = ($input["BANK_CODE"]<>'')?$input["BANK_CODE"]:$chkd[0]->bank_cd;
			$bank_reff_no = ($input["BANKREFNO"]<>'')?$input["BANKREFNO"]:$chkd[0]->bank_transaction_id;
