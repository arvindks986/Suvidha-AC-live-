...
class NominationController extends Controller
{
  ...
  //   }
    ...
    $message['TransectionAction']= 'Insert Personal Details ';
	...
	$pid        = ProfileModel::add_nomination_personal_detail($request->all());
      ...
      if(config('public_config.nomination_log'))
          ...
          $message['LogDescription']= 'Candidate Personal Details Error: '. $e;
  ...
  public function apply_nomination_step_2(Request $request,$id = 0)
  {	
...
//	echo "<pre>"; print_r($states); die("data");
	...
	$sql = "select m_ac.AC_NO as ac_no, m_ac.ST_CODE as st_code, m_ac.DIST_NO_HDQTR as district_no, CONCAT(AC_NAME) as ac_name, ELECTION_ID as election_id, ELECTION_TYPEID as election_type from `m_ac` inner join `m_election_details` on (`m_election_details`.`ST_CODE` = `m_ac`.`ST_CODE` and `m_election_details`.`CONST_NO` = `m_ac`.`AC_NO`) where `election_status` = 1 and `m_election_details`.`CONST_TYPE` = 'AC' group by `m_ac`.`AC_NO`, `m_ac`.`ST_CODE` order by m_ac.ST_CODE,m_ac.AC_NO ASC";
	$acs = DB::select($sql);
  ...
  public function save_step_2(NominationApplicationRequest $request){	
    ...
    $message['Module']= 'SUVIDHA';
    $message['TransectionType']= 'Online Nomination';
    $message['TransectionAction']= 'Election Details';
    ...
    try{ 
	 ...
	 // echo Session::get('nomination_id'); die;
      ...
      if(!$request->has('nomination_id')){
		...
		$destination_path = FileModel::get_file_path('uploads1/qrcode/'.$year.'/ac/'.$election_name.'/'.$st_code.'/'.$ac_no).'/'.$data['id'].'.png';
  ...
  public function apply_nomination_step_3(Request $request,$id = 0){
  ...
  public function save_step_3(NominationPart12Request $request){	
    ...
    $message['Module']= 'SUVIDHA';
    $message['TransectionType']= 'Online Nomination';
    $message['TransectionAction']= 'Nomination Part I/II';
  ...
  public function apply_nomination_step_4(Request $request,$id = 0){
  ...
  //save step 5
  public function apply_nomination_step_5(Request $request,$id = 0){
  ...
  public function apply_nomination_step_6(Request $request,$id = 0){
    $data                  = [];
    $data['breadcrumbs']    = [];
    $data['breadcrumbs'][]  = [
      'href'    => url('/'),
  ...
  public function apply_nomination_finalize(Request $request,$id = 0){
  ...
  public function save_nomination_finalize(Request $request,$id = 0){
  ...
  public function getAPSFromDetailsTB($nom){
	 ...
	 $res=DB::connection('mysql')->table('v_candidate_nomination_detail')
  ...
  //dd($Request->all());
	...
	if(count($payment_config) > 0){
		...
		//Condition will be increase as payment gateway will be increase
			if($nom[0]->st_code=='S25'){//West Bengol
		...
		} else if($nom[0]->st_code=='S23'){ // Tripura
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-tri';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
						$data['mobile']=trim($profile[0]->mobile);
						$data['city']= getdistrictbydistrictno($profile[0]->state,$profile[0]->district)->DIST_NAME;
						$data['candidate_id']=trim($profile[0]->candidate_id);
						$data['address']=trim(substr($profile[0]->address, 0, 90));
						$data['remarks']=trim('Payment');
						$data['amount']=trim($amt);
						
		} else if($nom[0]->st_code=='S15'){ // Meghalaya
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-meg';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
		...
		} else if($nom[0]->st_code=='S17'){ // Nagaland
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-nag';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
		...
		} else if($nom[0]->st_code=='S10'){ // Karnataka
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-kar';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
		...
		} else if($nom[0]->st_code=='S12'){ // Madhya Pradesh
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-mp';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
						...
						$payment_office_code =  DB::table('mp_district_mapping')
						->select('*')
						->where('DIST_NO', $dist_code)
						->first();
						...
						if($payment_office_code){
							$data['dist_code'] = $payment_office_code->OFFICE_CODE;
							$data['office_name'] = $payment_office_code->LOC_NAME;
						}else{
							$data['dist_code'] = '';
							$data['office_name'] = '';
						}	
			...
			} else if($nom[0]->st_code=='S16'){ // Mizoram
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-miz';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
			...
			} else if($nom[0]->st_code=='S20'){ // Rajasthan
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-raj';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
			...
			} else if($nom[0]->st_code=='S26'){ // Chhattisgarh
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-chh';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
			...
			} else if($nom[0]->st_code=='S29'){ // Telangana
						$data['isPaymentConfig']='YES';	
						$data['payState']=$nom[0]->st_code;
						$randomid = mt_rand(10000000,99999999); 
						$rurl =  url('/').'/payment-return-handle-tel';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
					...
					}else if($nom[0]->st_code=='S18'){	//Odisha

						$stateName	= DB::connection('mysql')
						->table('m_state')
						->select('ST_NAME')
						->where('st_code', '=', $nom[0]->st_code)
						->first();
						...
						$randomid = mt_rand(10000000,99999999);
						$rurl =  url('/').'/payment-return-handle-odi';
						$data['return_url']=trim($rurl);
						$data['reff_no']=trim($randomid);	
						$data['depositor_name']=trim($profile[0]->name);
						$data['email']=trim($profile[0]->email);
  ...
  public function getNominationStatus($non){
	...
	->table('v_candidate_nomination_detail')
	->select('*')
	->where('candidate_id', '=', \Auth::id())
  ...
  public function sendEmail($email, $message, $subject){ 
	...
	$to_email = $email;
	$subject="Email verification From ECI";
	$body= $message;
	$body.= "\n ". __('finalize.eci') ;
	$header = "From:ECI Candidate Portal <no-reply@eci.gov.in>\r\n" ;
	//$header.= "MIME-Version: 1.0\r\n";
	//mail($to_email, $subject, $body, $header);	
	mail($to_email, $subject, $body, 'suvidha.eci.gov.in');	
  }
  public function sendEmail_cand($email, $otp, $subject){ 
	
         $mailinfo = new \stdClass();
								  $mailinfo->from = 'no-reply@eci.gov.in';
								  $mailinfo->subject = 'Email verification';
								  $mailinfo->team = 'Suvidha | Election Commission Of India';
								  $mailinfo->name ='Candidate';
								  $mailinfo->mobile_otp = $otp;
								 
								  Mail::to($email)->send(new CandidateOtpMail($mailinfo));
  }
 ...
 public function getEmail(){
  ...
  public function getScrutuniDate($no){
	return DB::connection('mysql')->table('v_candidate_nomination_detail')->select('scrutiny_date')->where('nomination_no', '=', $no)->value('scrutiny_date'); 	
  }
   ...
   public function send_otp_on_email(){
	...
	$this->sendEmail_cand($email, $otp, $subject);
