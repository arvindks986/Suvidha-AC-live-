...
class UkPaymentController extends Controller
{    
...
//dd($input);
 ...
 $arrData = explode("|",$input['detpstr']);
	...
	if(isset($paymentData['challan_no']) && isset($paymentData['trans_id'])){
		 try {
			...
			$pay_status = $paymentData["status"];
			$pament_gateway_refrence_no = $paymentData["challan_no"];
			$dept_id = $paymentData["trans_id"];
			$amount = $paymentData["amount"];
			
			if(isset($paymentData['CHALLANTIMESTAMP']) && $paymentData['CHALLANTIMESTAMP']){
				$challan_timestamp = date('Y-m-d H:i:s',strtotime($paymentData['CHALLANTIMESTAMP']));
			}else{
				$challan_timestamp = '';
			}
			if(isset($paymentData['Bank_date']) && $paymentData['Bank_date']){
				$bank_timestamp = date('Y-m-d H:i:s',strtotime($paymentData['Bank_date'].' '.$paymentData['Bank_Time']));
			}else{
				$bank_timestamp = '';
			}
			$paymentconfirmationnumber = ($paymentData["ref_no"]<>'')?$paymentData["ref_no"]:$chkd[0]->payment_confirmation_number_cin;
			$bank_code = ($paymentData["bank_name"]<>'')?$paymentData["bank_name"]:$chkd[0]->bank_cd;
			$bank_reff_no = ($paymentData["ref_no"]<>'')?$paymentData["ref_no"]:$chkd[0]->bank_transaction_id;
