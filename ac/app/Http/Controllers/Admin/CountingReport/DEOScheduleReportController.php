...
class DEOScheduleReportController extends Controller {

	public $view_path     = "admin.countingReport.scheduleReport.deo";
	public $aro           = "aro";
	public $ropc          = "ropc";
	public $eci           = "eci";
	...
	public function scheduleReport(Request $request)
	{
	  ...
	  ->where('m_ac.DIST_NO_HDQTR',$district_no)
	  ...
	  {
         $d_val=$d_val.','.$data->AC_NO;  
	  }
	  $d_val=trim($d_val,',');
	  }	
      //print_r($d_val); die;
	  $heading_title = 'Scheduled Rounds Report';	  
	  $result=DB::select(DB::raw("SELECT scheduled_round AS S_ROUND,st_code AS STATE,ac_no AS AC_NO FROM round_master WHERE st_code='$state' AND ac_no IN ($d_val) order by ac_no"));
	  ...
	  return view($this->view_path.'.deo-schedule-report', ['user_data'=>$user_data,'m_ac'=>$m_ac,'data'=>$data,'result'=>$result,'state'=>$state,'urlpdf'=>$urlpdf,'urlexcel'=>$urlexcel,'ac_no'=>$ac_no,'heading_title'=>$heading_title]);
	}
	...
	public function scheduleReportPDF($ac_no="")
	{
	  ...
	  $pdf = PDF::loadView($this->view_path.'.deo-schedule-report-pdf', compact('result','data','ac_no','state','ac_name','user_data','heading_title'));
	  return $pdf->download($date.'-'.$state.'-deo-schedule-report.pdf');
	}
	...
	public function scheduleReportExcel($ac_no="")
	{
      ...
      $date = date('Y-m-d-H:i:s');
	  $type='csv';
      return Excel::create($date.'-'.$state.'-deo-schedule-report', function($excel) use ($data) {
