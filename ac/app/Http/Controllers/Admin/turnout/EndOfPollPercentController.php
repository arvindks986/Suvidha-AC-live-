...
class EndOfPollPercentController extends Controller
{

  public $base          = 'ro';
  public $folder        = 'eci';
  public $action_state  = 'eci/turnout/end-of-poll-percent';
  public $action_ac     = 'eci/turnout/end-of-poll-percent/state/ac';
  ...
  public function percent_state(Request $request)
  {
    ...
    //set title
    $title_array  = [];
    $data['heading_title'] = 'End of Poll Percent';
    ...
    return view($this->view_path . '.end_of_poll.state_percent', $data);
  ...
  public function percent_ac(Request $request)
  {
    try {
      $data = [];
      $default_phase = PhaseModel::get_current_phase();
      ...
      return view($this->view_path . '.end_of_poll.ac_percent', $data);
    } catch (\Exception $e) {
      dd($e);
      Log::error($e);
      return Redirect::to('/eci/dashboard');
  ...
  public function export_excel_percent_state(Request $request)
  {

    set_time_limit(6000);
    $data = $this->percent_state($request->merge(['is_excel' => 1]));

    $export_data = [];
    $export_data[] = [$data['heading_title']];

    $export_data[] = ['State', 'Total Percentage'];
    $headings[] = [];
  ...
  public function export_pdf_percent_state(Request $request)
  {
    $data = $this->percent_state($request->merge(['is_excel' => 1]));
    $name_excel = strtolower(str_replace([',', ': ', ' '], ['_', '-', '_'], $data['heading_title']));
    $pdf = \PDF::loadView($this->view_path . '.end_of_poll.state_percent_pdf', $data);
    return $pdf->download($name_excel . '_' . date('d-m-Y') . '_' . time() . '.pdf');
  }


  //export AC's
  public function export_excel_percent_ac(Request $request)
  {

    set_time_limit(6000);
    $data = $this->percent_ac($request->merge(['is_excel' => 1]));

    $export_data = [];
    $export_data[] = [$data['heading_title']];
    $export_data[] = ['The percentage is calculated on the basis of number of voters entered by RO after End of Poll*.'];
    $export_data[] = ['State', 'ac no', 'ac name', 'Total Percentage', 'Finalized By RO'];
    $headings[] = [];
  ...
  public function export_pdf_percent_ac(Request $request)
  {
    $data = $this->percent_ac($request->merge(['is_excel' => 1]));
    $name_excel = strtolower(str_replace([',', ': ', ' '], ['_', '-', '_'], $data['heading_title']));
    $pdf = \PDF::loadView($this->view_path . '.end_of_poll.ac_percent_pdf', $data);
    return $pdf->download($name_excel . '_' . date('d-m-Y') . '_' . time() . '.pdf');
  }
}  // end class