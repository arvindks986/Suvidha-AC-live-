...
use Validator;
use PDF;
class DeoPermissionReportController extends Controller {
    ...
    public function TimeWiseReport(Request $req)
    {
        if (Auth::check()) {
            ...
            $distvalue = DB::table('m_district')->where(['ST_CODE' =>$d->st_code,'DIST_NO'=>$d->dist_no])->select('DIST_NO','DIST_NAME')->first();
            $getAllAC=DB::table('m_ac')->select('AC_NO','AC_NAME')->where(array('DIST_NO_HDQTR'=>$d->dist_no,'ST_CODE'=>$d->st_code))->get()->toArray();
            $data=DB::table('permission_request as a')
                                    ...
                                    ->get()->toArray();
            return view('admin.ac.deo.Permission.DeoPermissionReport', ['report'=>$data,'user_data' => $d,'distvalue' => $distvalue,'allac'=>$getAllAC,'time'=>0,'ac_no'=>0]);
        }
    }
        ...
        if (Auth::check()) {
            ...
            $data1=$data->get()->toArray();
             ...
             }else
             {
                    ...
                    $pdf = PDF::loadView('admin.ac.deo.Permission.timewisereport', ['user_data' => $d, 'records' => $data1]);
            ...
            $rules = [
             ...
             if ($validator->passes()) {
                ...
                $data=DB::table('permission_request as a')
                        ...
                        ->select('a.ac_no','f.DIST_NAME',

                        //DB::raw("GROUP_CONCAT(DISTINCT g.AC_NAME SEPARATOR ',') as 'ac_name'"),

                         DB::raw("(SELECT  GROUP_CONCAT(DISTINCT m.AC_NAME SEPARATOR ',')  FROM m_ac m WHERE 
 FIND_IN_SET(m.AC_NO, a.ac_no) > '0' AND `m`.`ST_CODE` = `a`.`st_code`)AS 'ac_name'"),
               ...
               // dd(DB::getQueryLog());
                 ...
                 else if($req->ac == 'all')
                 {
                     $data=DB::table('permission_request as a')
                        ...
                        ->select('a.ac_no','f.DIST_NAME',//DB::raw("GROUP_CONCAT(DISTINCT g.AC_NAME SEPARATOR ',') as 'ac_name'"),
...
//                        ->where('a.ac_no','!=','')
             ...
             return view('admin.ac.deo.Permission.permissionsummaryreport', ['user_data' => $d,'distvalue' => $distvalue,'allac'=>$getAllAC,'dist_no' => $req->dist,'ac_no' => $req->ac,'summarydata'=>$data]);
             }
             else
             {
...
//                        ->where('a.ac_no','!=','')
                ...
                ->get()->toArray();
                //dd(DB::getQueryLog());
            return view('admin.ac.deo.Permission.permissionsummaryreport', ['user_data' => $d,'distvalue' => $distvalue,'allac'=>$getAllAC,'summarydata'=>$summarydata]);
        }
        }
        else {
            return redirect('/officer-login');
...
//                        ->where('a.ac_no','!=','')
                ...
                {
                    $pdf = PDF::loadView('admin.ac.deo.Permission.permissionsummaryreportPDF', ['user_data' => $d, 'records' => $data]);
                    return $pdf->download('report' . $cur_time . '.pdf');
...
public function ReportTimesview(Request $req) {
    if (Auth::check()) {
            ...
            if($req->method() == 'POST')
        {
                ...
                return view('admin.ac.deo.Permission.DeoPermissionReport', ['report'=>$data,'user_data' => $d,'distvalue' => $distvalue,'allac'=>$getAllAC,'time'=>$req->time,'ac_no'=>$req->ac]);
        }
        else
        {
             $data=DB::table('permission_request as a')
...
public function CustomTimeWiseReport(Request $req)
{
    if (Auth::check()) {
        ...
        if($req->method() == 'GET')
        {
            return view('admin.ac.deo.Permission.DeoCustomPermissionReport', ['user_data' => $d,'distvalue' => $distvalue,'allac'=>$getAllAC]);
        }
        else
        {
           $cur_time = Carbon::now();
...
//})->export('xlsx');
             ...
             {
                 if (!empty($_REQUEST['dist']) && ( !empty($_REQUEST['ac'])) && ( !empty($_REQUEST['time']))) {
                    $pdf = PDF::loadView('admin.ac.deo.Permission.customtimewisereport', ['user_data' => $d, 'records' => $data1]);
                    return $pdf->download('report' . $cur_time . '.pdf');
                  }
             }
             }
        ...
        $data = DB::table('permission_request as a')
                   ...
                   ->join('permission_master as m','m.id','=','d.permission_type_id')
                    ...
                    $result=$data->get()->toArray();
                        return view('admin.ac.deo.Permission.permissionsummaryreportdetails', ['distvalue' => $distvalue,'allac'=>$getAllAC,'ac'=>$req->ac,'user_data' => $d,'countdetails'=>$result]);
    }
    else
    {
        return redirect('/officer-login');
