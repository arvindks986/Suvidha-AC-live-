<?php  
    ...
    use App\models\Candidate\CandidateCriminalLogModel;
        ...
        {      
            ...
            $ele_details=$this->commonModel->election_detailsac($d->st_code,$d->ac_no,$d->dist_no,$d->id,'AC');
             $record = DB::table('m_election_details')->where('ST_CODE', $ele_details->ST_CODE)->where('CONST_NO', $ele_details->CONST_NO)
                  ...
                  $message['TransectionAction']= 'Insert Nomination Details';
                ...
                //  'fhname.required'=>'Please enter father/husband name in hindi',
                 ...
                 // 'cand_mobile.required'=>'Please enter validate mobileno',
                  ...
                  'cand_category.required'=>'Please select candidate category',
                   'part_no.required' => 'Please select Part Number',
          'serial_no.required' => 'Please select Serial Number',
                ]
            ); 
      ...
      //Validation for file type and file size if candidate opted Yes 
           ...
           $shares = DB::table('candidate_nomination_detail')
            ...
            $candPersonalData = array(
              ...
              'candidate_residance_part_no' => $this->xssClean->clean_input(Check_Input($request->input('part_no'))),
              'candidate_residance_serial_no' => $this->xssClean->clean_input(Check_Input($request->input('serial_no'))),
      ...
      /*-------Transaction Start-----------*/
            ...
            if($cid != ''){
                ...
                if(!empty($request->file('profileimg'))){
                  ...
                  if($ext!='jpg')
                  {
                    \Session::flash('error_messageis', 'The Profile Image Format: .jpg ');
                    return Redirect::back()->withInput($request->all());
                  }
                    $cyear = date('Y');
                    
                    //Move Uploaded File
                     $string=trim(substr(str_replace(' ','',$candName),0,5));
                    $newname=preg_replace('/[^A-Za-z0-9\-]/', '', $string);
         ...
         //end by waseem   
          ...
          if(!file_exists($affidavit_path)){
               ...
               if(config('public_config.nomination_log'))
                 {
                  $message['LogDescription']= 'File Not Uploaded';
        ...
        DB::commit();
              if($cand_mob!='') {
                 ...
                 if(config('public_config.nomination_log'))
                    {
                      $message['LogDescription']= 'Nomination  Successfully, CandidateID ' .$cid.' NominationID: '.$nom_id;
        ...
        DB::rollback();
         if(config('public_config.nomination_log'))
        {
          $message['LogDescription']= 'Nomination Record Rollaback';
          $message['TransectionStatus']= 'Failed';
...
public function updatenomination(request $request, $nomid){
  ...
  if(Auth::check()){ 
               ...
               $countdate_btn= date('Y-m-d', strtotime($getlastwithdrawl_stateACwise->LDT_WD_CAN));
               ...
               $message['TransectionAction']= 'Update Candidate Profile Details';

                 if(date("Y-m-d") <= $countdate_btn){
                 $criminalcheck = 'required|in:0,1';
                    }else{
      ...
      //Validation for file type and file size if candidate opted Yes 
         ...
         // $checkcriminal=$request->input('is_criminal');
          ...
          $checkcriminal=($request->has('is_criminal'))? $request->input('is_criminal'):$CandID->is_criminal;
           ...
           $checkcontesting_cand=CandidateNomination::where(['candidate_id' =>$nom->candidate_id,'ac_no' =>$ele_details->CONST_NO,'election_id' =>$ele_details->ELECTION_ID,'st_code' =>$d->st_code])->where('party_id', '!=' ,'1180')->where('finalaccepted','=', 1)->where('application_status', '=', 6)->first();

              $record = DB::table('candidate_criminal_report')->where('ST_CODE', $d->st_code)
                    ->where('ac_no', $d->ac_no)
                    ->where('candidate_id', $nom->candidate_id)->count();
            

           if($checkcriminal==1 && !empty($checkcontesting_cand))
           {
           // dd("sdsdsdsds"); 
                  ...
                  'candidate_id'      =>$checkcontesting_cand->candidate_id, 
                  'nom_id'            =>$checkcontesting_cand->nom_id,
                  'st_code'           =>$checkcontesting_cand->st_code,
                  'ac_no'             =>$checkcontesting_cand->ac_no,
                  'election_id'       =>$checkcontesting_cand->election_id,
                  'check_1'           =>'0',
                  'check_2'           =>'0', 
                  'check_3'           =>'0',
                  'status'            =>'0',
                ...
                $sts_ca = array('finalaccept_ca' =>'1','nom_id'=>$checkcontesting_cand->nom_id);
                  $ins = DB::table('candidate_criminal_report')->where('ST_CODE', $d->st_code)->where('ac_no', $d->ac_no)->where('candidate_id', $checkcontesting_cand->candidate_id)->update($sts_ca);

                    }


           }elseif($checkcriminal==0 && !empty($checkcontesting_cand))
           {

              if($record > 0){

                $sts_ca = array('finalaccept_ca' =>'0','nom_id'=>$checkcontesting_cand->nom_id);
                           $ins = DB::table('candidate_criminal_report')->where('ST_CODE', $d->st_code)->where('ac_no', $d->ac_no)->where('candidate_id', $checkcontesting_cand->candidate_id)->update($sts_ca);
    ...
    $candPersonalData = array(
                ...
                'is_criminal'=>$this->xssClean->clean_input($checkcriminal),
      ...
      $candNomData = array(
        ...
        'cand_party_type'=> $partytype
            ...
            if(!empty($request->file('profileimg'))){
              ...
              $ext=$request->file('profileimg')->getClientOriginalExtension();
                    ...
                    if($ext!='jpg')
                    {
                      \Session::flash('error_messageis', 'Profile Image Format Allowed: .jpg ');
                      return Redirect::back()->withInput($request->all());
                    }
              $cyear = date('Y');
              $string=trim(substr(str_replace(' ','',$candName),0,5));
        ...
        $cyear = date('Y');
         ...
         //end by waseem   
          ...
          if(!file_exists($affidavit_path)){
            ...
            if($request->file('affidavit') != '')
            {
               CandidateCriminalLogModel::clone_record($nom->candidate_id);
        ...
        if(!$log_updated)
          \App\models\Candidate\CandidateLogModel::clone_record($nomid);
            ...
            if(config('public_config.nomination_log'))
                 {
               $message['LogDescription']= 'CandidateID: '.$nom->candidate_id. ' NominationID: '. $nomid;
               $message['TransectionStatus']= 'SUCCESS';
...
public function candstoreaffidavit(request $request){
  //ini_set('max_execution_time', -1);
       if(Auth::check()){ 
            ...
            $message['Module']= 'ENCORE';
            $message['TransectionType']= 'Offline Nomination';
            $message['TransectionAction']= 'Upload Affidavit';
      ...
      // dd($ele_details);
        ...
        $destinationPath ='uploads1/acaffidavit/E'.$electionid.'/'.$cyear.'/'.$electionName.'/'.$stateid .'/'.$cons_no;
          ...
          if(!file_exists($affidavit_path)){
             if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'Affidavit Not uploaded  NominationID: '.$nom_id;
          $message['TransectionStatus']= 'Failed';
        ...
        /*  if(!empty($getAffidavitDetails) ){
          ...
          if(config('public_config.nomination_log'))
        {
          $message['LogDescription']= 'Affidavit uploaded Successfully NominationID: '.$nom_id;
          $message['TransectionStatus']= 'SUCCESS';
  ...
  public function storecountercandaffidavit(request $request){
    if(Auth::check()){ 
      $user = Auth::user();
            ...
            $message['Module']= 'ENCORE';
            $message['TransectionType']= 'Offline Nomination';
            $message['TransectionAction']= 'Insert Counter Affidavit ';
    ...
    $cyear = date('Y');
     ...
     //end by waseem   
      ...
      if(!file_exists($affidavit_path)){
         if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'Counter Affidavit Not uploaded  NominationID: '.$nom_id;
          $message['TransectionStatus']= 'Failed';
    ...
    if(config('public_config.nomination_log'))
           {
          $message['LogDescription']= 'Counter Affidavit uploaded Successfully NominationID: '.$nom_id;
          $message['TransectionStatus']= 'SUCCESS';
 ...
 public function candidateiscriminal($lastid=''){
  ...
  public function uploadiscriminal(request $request){  
          ...
          $ele_details=$this->commonModel->election_detailsac($user->st_code,$user->ac_no,$user->dist_no,$user->id,'AC');
            ...
            $message['Module']= 'ENCORE';
            $message['TransectionType']= 'Offline Nomination';
            $message['TransectionAction']= 'Update Criminal Antecedents';
  ...
  //dd($getAffidavitDetails);
    ...
    $cyear = date('Y');
     ...
     //end by waseem   
      ...
      if(!file_exists($affidavit_path)){
        // if(config('public_config.nomination_log'))
        //    {
        //   $message['LogDescription']= 'File is not uploaded.  CandidateID: '.$cid;
        //   $message['TransectionStatus']= 'Failed';
        ...
        //   }
          ...
          if(isset($getAffidavitDetails)  and ($getAffidavitDetails)){
            ...
            if($request->file('affidavit') != ''){
              CandidateCriminalLogModel::clone_record($cid);
              $updateNomDetail = DB::update('update candidate_criminaluploads set path ="'. $affidavit_path. '" where candidate_id = ' .$cid);
            }
          }else{
            $insData = array(
     ...
     if(config('public_config.nomination_log'))
        {
          $message['LogDescription']= 'Criminal Antecedents uploaded Successfully CandidateID: '.$cid;
          $message['TransectionStatus']= 'SUCCESS';
   ...
   public function countffidavitdetails($nom_id)
  {

    if (Auth::check()) {
      $user = Auth::user();
      $d = $this->commonModel->getunewserbyuserid($user->id);
      $ele_details = $this->commonModel->election_detailsac($d->st_code, $d->ac_no, $d->pc_no, $d->id, 'AC');
      $check_finalize = candidate_finalizebyro($ele_details->ST_CODE, $ele_details->CONST_NO, $ele_details->CONST_TYPE);
      ...
      // }
      $list = $this->roacmodel->getcountffidavitebynomid($ele_details->ST_CODE, $ele_details->CONST_NO, $nom_id);

      $data['user_data'] = $d;
      $data['ele_details'] = $ele_details;
      $data['list'] = $list;
   
     return view('admin.ac.ro.count_affidavitdetails', $data);
...
public function getaffidavit(Request $request)
  {

    $nomid = $request->input('nom_id');


   // $i = DB::table('candidate_affidavit_detail')->where('nom_id', $nomid)->get()->toarray();;
    $i = DB::table('candidate_affidavit_detail')->where('nom_id', $nomid)->where('is_deleted','1')->get()->toarray();
  if(!empty($i)){
    $response = "<table border='0' width='100%'>";
    foreach ($i as $ias) {
      $explodeis = explode(" ", $ias->created_at);
      // dd($explodeis);
      ...
      $response .= "<tr>";
      $response .= "<td>Date & Time : </td><td>" . $dateis . " & <b>" . $time . "</b></td>";
      $response .= "</tr>";


      $response .= "<tr>";
      $response .= "<td>Files : </td><td><a href=" . asset($ias->affidavit_path) . " target='_blank'>Form26</a></td>";
      $response .= "</tr>";

      // $response .= "<tr>"; 
      // $response .= "<td>Email : </td><td>".$time."</td>"; 
      // $response .= "</tr>";
  ...
  public function deleteaffidavit(Request $request)
  {
    ...
    if (Auth::check()) {
      ...
      //  $ele_details = $this->commonModel->election_details($d->st_code, $d->ac_no, $d->pc_no, $d->id, 'PC');
         $ele_details = $this->commonModel->election_detailsac($d->st_code, $d->ac_no, $d->pc_no, $d->id, 'AC');
      $affidavit_id = $this->xssClean->clean_input($request->input('affidavit_id'));
      $nom_id = $this->xssClean->clean_input($request->input('nom_id'));

      $nom_id_decode = base64_decode($nom_id);
      
      //if ($marks == 11) {
       // $st = array('is_deleted' => '0');
        $st = array('is_deleted' => '0','updated_at' => date('Y-m-d H:i:s'));
        $i = DB::table('candidate_affidavit_detail')->where('nom_id', $nom_id)
        ->where('candidate_affidavit_id', $affidavit_id)
        ->update($st);

        
       \Session::flash('success_mes', 'Affidavit Deleted');

        
     
    
        return Redirect::to('roac/candidateaffidavit');
      //  return Redirect::to('ropc/countffidavitdetails');
        
      // } else {
      //  //Session::flash('error_mes', 'please select duplicate status');
      //  //return Redirect::to('ropc/getaffidavit');
      // }
    } else {
      return redirect('/officer-login');
    }
