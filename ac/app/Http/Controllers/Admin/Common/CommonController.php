...
//echo $url; dd();
        ...
        $elector_information = $this->get_cdac_file(['url' => $url,'epic'=> $epic_no]);
       // echo "<pre>";
        //print_r($elector_information[0]->content);die;
        //return $elector_information;
        
        
        // $i = 0;
        // while(isset($elector_information) && $elector_information[0]->content == 0 && $i < 3){
        //    // $elector_information = $this->get_cdac_file(['url' => $url]);
        //     $i++;
        // }
//echo "<pre>"; print_r($elector_information); exit;
    if(isset($elector_information) && !empty($elector_information)){
     
        //$data = (array)$elector_information->response->docs[0];
        $data = (array)$elector_information[0]->content;
       // echo "<pre>"; print_r($elector_information); exit;
        //$pass_key = $this->get_pass_key($data['st_code'].$epic_no);
...
public function getEpicDetails($epic,$st_code) { 
  ...
  $key = base64_decode('P79vtNtk/WZaAXsQKCHClA==');
...
$encodedEncryptedData = base64_encode( $ciphertext . $tag );
...
// $key="P79vtNtk/WZaAXsQKCHClA==";
// $dateis=date("Y-m-d H:i:s");
// $generatedPublicKey = $epicno . ":" . $dateis . ":" . getRandomString();
//    // openssl_public_encrypt($arr["UserName"],$UserName,$pub_key_string);
//     openssl_encrypt($epicno,$method,$first_key, OPENSSL_RAW_DATA ,$iv);
