  ...
  public function electorsDeoPollingstationStore(Request $request){
	 ...
	 {  
		...
		/*	'const_no'=>$request->ac_no[$i],*/
		  ...
		  //dd($elector_data);
		 $checkelectorData = DB::table('elector_details')->where('st_code', $st_code)->where('dist_no', $dist_no)->where('ac_no', $ac_no)->first();
		 //dd($checkelectorData);
		 if(!empty($checkelectorData)){
			$elector_Updatedata = array(
				'election_id'=>$election_id,
			/*	'const_no'=>$request->ac_no[$i],*/
				'const_type'=>$CONST_TYPE,
				'st_code'=>$st_code,
			...
			$n = DB::table('elector_details')->where('st_code', $st_code)->where('dist_no', $dist_no)->where('ac_no', $ac_no)->update($elector_Updatedata);
		   }else{
           $n = DB::table('elector_details')->insert($elector_Insertdata);
		}
		//dd(DB::getQueryLog());
	 }
