...
class CEOScheduleReportController extends Controller {
	...
	public $view_path     = "admin.counting.reports.ceo";
	public $aro           = "aro";
	public $ropc          = "ropc";
	public $eci           = "eci";
	public $ceo           = "ceo";
	...
	public function scheduleReport(Request $request)
	{
	  ...
	  $sql = DB::table('m_ac');
	  $sql->leftjoin('m_election_details as election',[
		              ['election.CONST_NO', '=','m_ac.AC_NO'],
		              ['election.ST_CODE', '=','m_ac.ST_CODE'],
		        ]);
	  $sql->where('m_ac.ST_CODE',$state);
	  if(!empty($dist_no)){
	  	 $sql->where('m_ac.DIST_NO_HDQTR', '=', $dist_no);
	  }
	  $sql->where('election.CONST_TYPE', '=', 'AC');
	  $sql->where('election.ELECTION_ID', '=', $this->election_id);
	  $sql->orderBy('m_ac.AC_NO','ASC');
	  $data['m_ac']=$sql->get();
	  
	  $heading_title = 'Scheduled Rounds Report';	
	  if(!empty($dist_no)){
	  	$result=DB::select(DB::raw("SELECT scheduled_round AS S_ROUND,st_code AS STATE,ac_no AS AC_NO FROM round_master WHERE st_code='$state' and dist_no='$dist_no' order by ac_no"));
	  }else{
	...
	//    dd($this->view_path);
	  $pdf = PDF::loadView($this->view_path.'.ceo-schedule-report-pdf', compact('result','date','ac_no','state','ac_name','user_data','heading_title'));
	...
	public function scheduleReportExcel($ac_no="")
	{
	  ...
	  $dataResult=[];
	  $headings[]=['Schedule Report Excel'];
	  $export_data[]=[];
