            ...
            try {
                ...
                if($ac_val){
					if($ac_val->AC_TYPE<>'GEN'){
						...
						$ac_name1 = $ac_val->AC_NAME . ' ' . $nac_no.' ('.$ac_val->AC_TYPE.') ';
					}else{
    ...
    function getForm21CPdf() {
        if (Auth::check()) {
            try {
                ...
                if($ele_details->ELECTION_TYPEID==4){
                    $pdf = PDF::loadView('admin.countingReport.form21c.form21d-report-pdf', ['user_data' => $user,'ac_state'=>$state_name, 'acname' => $ac_name, 'ac_name1' => $ac_name1, 'state' => $cand_state,'dist'=>$can_district, 'wincan' => $get_win_candidate]);
                    return $pdf->download($date . '-form-21d' . '.' . 'pdf');
                }else{
                    $pdf = PDF::loadView('admin.countingReport.form21c.form21c-report-pdf', ['user_data' => $user,'ac_state'=>$state_name, 'acname' => $ac_name, 'ac_name1' => $ac_name1, 'state' => $cand_state,'dist'=>$can_district, 'wincan' => $get_win_candidate]);
            ...
            try {
                ...
                if ($request->hasFile('form21')) {
                    ...
                    if($ele_details){
                        ...
                        $file_path = '/uploads/form' . $request->form_type . '/' . $ele_details->CONST_TYPE . '/' . date('Y') . '/' . $fileNameToStore;
                        ...
                        $request->file('form21')->move(public_path('/uploads/form' . $request->form_type . '/' . $ele_details->CONST_TYPE . '/' . date('Y') . '/'), $fileNameToStore);
                    }else{
