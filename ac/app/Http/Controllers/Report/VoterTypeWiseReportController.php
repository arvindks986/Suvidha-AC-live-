        ...
        if (Auth::check()) {
            try {
                if ($stateid != '' && $acno != '' && $party_id != '') {
                    ...
                    $evm_vote = 0;
                    if (count($list_record) > 0) {
                        $export_data[] = ['SL.No','State Name','AC Name', 'Candidate Name', 'Party Name','EVM Vote','Postal Vote','Total Vote'];
                        $headings[] = [];
                        ...
                        foreach ($list_record as $dcode => $dval) {
                            ...
                            $export_data[] = [
                                $i,
                                $state_name,
                                $ac_name->AC_NO.'-'.$ac_name->AC_NAME,
                                $dval->candidate_name,
                                $dval->party_name . ' (' . $dval->party_abbre . ')',
                                $evm_vote,
                                $dval->postalballot_vote,
                                $dval->total_vote,

                     
                              ];

                            // $mArray[] = array('SL.No' => $i,
                            //  'State Name' => $state_name,
                            //   'AC Name' => $ac_name->AC_NO.'-'.$ac_name->AC_NAME,
                            //    'Candidate Name' => $dval->candidate_name,
                            //    'Party Name'=>$dval->party_name . ' (' . $dval->party_abbre . ')',
                            //     'EVM Vote' => "$evm_vote",
                            //      'Postal Vote' => "$dval->postalballot_vote",
                            //      'Total Vote' => "$dval->total_vote"
                            //     );
                            $i++;
                        }
                        //$data = json_decode(json_encode($mArray), true);
                        $date = date('Y-m-d');
$name_excel=$date.'-candidate-wise-report';
...
$ac_name = $ac_name->AC_NO.'-'.$ac_name->AC_NAME;
                ...
                $totelctroll = DB::table('electors_cdac')->select(DB::raw("SUM(electors_total) AS totelectors"))->where('ac_no', '=', $acno)->where('st_code', '=', $stateid)->first();
                if($totelctroll){
                    $totelctroll = $totelctroll->totelectors;
                }
               
                $totelservicectroll = DB::table('electors_cdac')->select(DB::raw("SUM(electors_service) AS totserviceelectors"))->where('ac_no', '=', $acno)->where('st_code', '=', $stateid)->first();
