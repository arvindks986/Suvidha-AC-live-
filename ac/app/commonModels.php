...
class commonModels extends Model
{
	...
	{
		$add = DB::connection('suivhdaaclivetest')->table($table)->insert($data);
		return $add;
	}
	public function selectAll($table, $order, $orderby)
	{
		$get = DB::connection('suivhdaaclivetest')->table($table)->select('*')->orderBy($order, $orderby)->get();;
		return ($get);
	}
	public function selectone($table, $field, $fieldvalue)
	{
		$g = DB::connection('suivhdaaclivetest')->table($table)->select('*')->where($field, $fieldvalue)->first();

		return ($g);
	}
	public function updatedata($table, $field, $fieldvalue, $ndata)
	{
		$g = DB::connection('suivhdaaclivetest')->table($table)->where($field, $fieldvalue)->update($ndata);

		return ($g);
	}
	public function removerecord($table, $field, $fieldvalue)
	{
		$g = DB::connection('suivhdaaclivetest')->table($table)->where($field, $fieldvalue)->delete();
		return ($g);
	}
	public function countrecords($table, $field, $fieldvalue)
	{
		$cnt = DB::connection('suivhdaaclivetest')->table($table)->select('*')->where($field, $fieldvalue)->get()->count();
		return ($cnt);
	}
	public function getuserbyuserid($uid)
	{
		$data = DB::connection('suivhdaaclivetest')->table('officer_master')->where('officer_id', $uid)->first();
		return Auth::user();
	}
	public function getunewserbyuserid($uid)
	{
		$data = DB::connection('suivhdaaclivetest')->table('officer_login')->where('id', $uid)->first();

		return $data;
	}
	public function getroleprevilagebyroleid($roleid)
	{
		$data = DB::connection('suivhdaaclivetest')->table('role_previlage')->where('role_id', $roleid)->get();
		return $data;
	}

	public function getallmodule()
	{
		$data = DB::connection('suivhdaaclivetest')->table('module_master')->get();
		return $data;
	}
	...
	{
		$get = DB::connection('suivhdaaclivetest')->table('election_master')->get();
		return ($get);
	}
	public function getecctionBYid($id)
	{
		$r = DB::connection('suivhdaaclivetest')->table('election_master')->where('election_id', $id)->first();
		return ($r);
	}
	public function getallschedule()
	{
		$get = DB::connection('suivhdaaclivetest')->table('m_schedule')->select('*')->where('CURRENTELECTION', 'Y')->get();
		return ($get);
	}
	public function getschedulebyid($id)
	{
		$get = DB::connection('suivhdaaclivetest')->table('m_schedule')->select('*')->where('CURRENTELECTION', 'Y')->where('SCHEDULEID', $id)->first();
		return ($get);
	}

	public function getallstate()
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_state')->orderBy('ST_CODE', 'ASC')->get();
		return ($g);
	}
	public function getstatebystatecode($st)
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_state')->where('ST_CODE', $st)->first();
		return ($g);
	}
	public function getalldistrictbystate($st)
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_district')->where('ST_CODE', $st)->get();
		return ($g);
	}
	public function getdistrictbydistrictno($st, $disno)
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_district')->where('ST_CODE', $st)->where('DIST_NO', $disno)->first();
		return ($g);
	}
	public function getacbystate($st)
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_ac')->where('ST_CODE', $st)->orderBy('AC_NO', 'ASC')->get();
		return ($g);
	}
	public function getacbyacno($st, $acno)
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_ac')->where('ST_CODE', $st)->where('AC_NO', $acno)->first();
		return ($g);
	}
	public function getallacbypcno($st, $pcno)
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_ac')->where('ST_CODE', $st)->where('PC_NO', $pcno)->first();
		return ($g);
	}
	public function getpcbystate($st)
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_pc')->where('ST_CODE', $st)->get();
		return ($g);
	}
	public function getpcbypcno($st, $pcno)
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_pc')->where('ST_CODE', $st)->where('PC_NO', $pcno)->first();
		return ($g);
	}
	...
	{
		$getAclist = DB::connection('suivhdaaclivetest')->table('m_ac')->where('ST_CODE', $stcode)->where('DIST_NO_HDQTR', $disttno)->orderBy('AC_NO', 'asc')->get();
		return $getAclist;
	}
	...
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_party')->where('CCODE', $party_id)->first();
		return ($g);
	}
	public function getsymbol($sid)
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_symbol')->where('SYMBOL_NO', $sid)->first();
		return ($g);
	}
	public function getelectionlist($st, $acno, $ctype)
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_election_details')->where('ST_CODE', $st)->where('CONST_TYPE', strtoupper($ctype))->where('CONST_NO', $acno)->get();

		return ($g);
	}
	public function getcurrentelectiongroup($st)
	{
		$list = DB::connection('suivhdaaclivetest')->table('m_cur_elec')
	...
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_cur_elec')->where('ST_CODE', $st)->where('ConstType', strtoupper($ctype))->get();
		return ($g);
	}
	function election_details_cons($st_code, $consno, $ctype, $offcer_level = '')
	{
		if ($offcer_level == '')
			$rec = DB::connection('suivhdaaclivetest')->table('m_election_details')->where('ST_CODE', $st_code)->where('CONST_NO', $consno)->where('CONST_TYPE', strtoupper($ctype))->first();
		elseif ($offcer_level == "CEO")
			$rec = DB::connection('suivhdaaclivetest')->table('m_cur_elec')->where('ST_CODE', $st_code)->get();
		//dd($rec);
		return $rec;
	}
	function Audit_log_data($app_id, $user_id, $table_name, $id, $column_name, $old_value, $new_value, $ipaddress, $procedure_name, $trigger_name, $event_type, $event_status, $create_date)
	{
		...
		$add = DB::connection('suivhdaaclivetest')->table('audit_log_table')->insert($n_data);
		return $add;
	}
	public function currentelectiondetails($st_code, $cons_type, $election_id)
	{
		$req = DB::connection('suivhdaaclivetest')->table('m_election_details')->where('ST_CODE', $st_code)->where('CONST_TYPE', strtoupper($cons_type))->where('ELECTION_ID', $election_id)->get();

		return ($req);
	}
	public function allstatus()
	{
		$get = DB::connection('suivhdaaclivetest')->table('m_status')->select('*')->orderBy('id', 'ASC')->get();;
		return ($get);
	}
	public function getnameBystatusid($id)
	{
		$g = DB::connection('suivhdaaclivetest')->table('m_status')->select('status')->where('id', $id)->first();
		return ($g->status);
	}
	function election_detailsac($st_code, $acno, $dist_no, $id, $offcer_level = '')
	{
		if ($offcer_level == 'ECI' || $offcer_level == 'ECI-OFFICE') {
			$list = DB::connection('suivhdaaclivetest')->table('m_election_details')
	...
	{
		$list = DB::connection('suivhdaaclivetest')->table('candidate_personal_detail')
	...
	{
		$data = DB::connection('suivhdaaclivetest')->table('pd_schedulemaster')->select('state_phase_no  as total_phase')->where('st_code', $state_code)->orderBy('state_phase_no', 'DESC')->limit(1)->first();
		if ($data) {
