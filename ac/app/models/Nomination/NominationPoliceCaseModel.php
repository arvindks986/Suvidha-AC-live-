...
class NominationPoliceCaseModel extends Model
{
  ...
  $xss = new xssClean;
   ...
   $object->case_no        =  preg_replace('/[^a-zA-Z0-9\s]/', '', $data['case_no']); 
    $object->police_station = preg_replace('/[^a-zA-Z0-9\s]/', '', $data['police_station']);
    $object->st_code          = $data['st_code']; 
    $object->case_dist_no       = $data['district'];  
    $object->convicted_des      = preg_replace('/[^a-zA-Z0-9\s]/', '', $data['convicted_des']);  
    $object->date_of_conviction =  date('Y-m-d', strtotime($data['date_of_conviction']));
    $object->court_name         = preg_replace('/[^a-zA-Z0-9\s]/', '', $data['court_name']);
    $object->punishment_imposed           = preg_replace('/[^a-zA-Z0-9\s]/', '', $data['punishment_imposed']);
    $object->date_of_release              = date('Y-m-d', strtotime($data['date_of_release'])); 
    $object->revision_against_conviction  = preg_replace('/[^a-zA-Z0-9\s]/', '', $data['revision_against_conviction']);
    $object->revision_appeal_date         = date('Y-m-d', strtotime($data['revision_appeal_date']));
    $object->rev_court_name               = preg_replace('/[^a-zA-Z0-9\s]/', '', $data['rev_court_name']);
    $object->status                       = $xss->clean_input($data['status']);
    $object->revision_disposal_date       = date('Y-m-d', strtotime($data['revision_disposal_date']));
    $object->revision_order_description   = preg_replace('/[^a-zA-Z0-9\s]/', '', $data['revision_order_description']);
    $object->is_deleted   = 0;
    $object->save();
	return $object['id'];
  }
  ...
  public static function add_police_case_ro($data = array(), $nomination_id, $candidate_id){ //echo "<pre>"; print_r($data); die;
   $xss = new xssClean;
    ...
    $object->nomination_id  = $nomination_id; 
    $object->case_no        = $xss->clean_input($data['case_no']); 
    $object->police_station = $xss->clean_input($data['police_station']); 
    $object->st_code          = $xss->clean_input($data['st_code']); 
    $object->case_dist_no       = $xss->clean_input($data['district']);  
    $object->convicted_des      = $xss->clean_input($data['convicted_des']);  
    $object->date_of_conviction =  date('Y-m-d', strtotime($data['date_of_conviction']));
    $object->court_name         = $xss->clean_input($data['court_name']);
    $object->punishment_imposed           = $xss->clean_input($data['punishment_imposed']);
    $object->date_of_release              = date('Y-m-d', strtotime($data['date_of_release'])); 
    $object->revision_against_conviction  = $xss->clean_input($data['revision_against_conviction']);
    $object->revision_appeal_date         = date('Y-m-d', strtotime($data['revision_appeal_date']));
    $object->rev_court_name               = $xss->clean_input($data['rev_court_name']);
    $object->status                       = $xss->clean_input($data['status']);
    $object->revision_disposal_date       = date('Y-m-d', strtotime($data['revision_disposal_date']));
    $object->revision_order_description   = $xss->clean_input($data['revision_order_description']);
    $object->is_deleted   = 0;
    $object->save();
	return $object['id'];
  }
