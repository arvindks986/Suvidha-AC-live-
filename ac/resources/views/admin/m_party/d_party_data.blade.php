...
<div class="container-fluid">
  ...
  <div class="row">
    <div class="col text-left">
      <label><b>Total records</b> : {{ \App\Http\Controllers\Admin\EciMPartyMasterData::getTotalDPartyRecords() }} </label>
      </div>
  </div>
  <div class="row">
      <div class="col text-left">
      <label><b>Total New</b> : {{ \App\Http\Controllers\Admin\EciMPartyMasterData::EciDPartyNewData() }}</label>
      </div>
  </div>
  <div class="row">
    <div class="col text-left">
      <label><b>Total Modified</b> : {{ \App\Http\Controllers\Admin\EciMPartyMasterData::EciDPartyModifiedData() }}</label>
    </div>
  </div>
  ...
  <div class="col text-right">
    @if(count($results)>0)
    <span class="report-btn" id="export-csv-btn"><a class="btn btn-primary" href="{{url('eci/d_party_export_to_excel',['from_date'=>$from_date,'from_to'=>$from_to,'rp_str'=>$rp_str])}}" title="Download Excel" target="_blank">Export Excel</a></span>
    <span class="report-btn" id="export-pdf-btn"><a class="btn btn-primary" href="{{url('eci/d_party_export_to_pdf',['from_date'=>$from_date,'from_to'=>$from_to,'rp_str'=>$rp_str])}}" title="Download PDF" target="_blank">Export PDF</a></span>
    @endif
  </div>
    </div>
...
<script type="text/javascript">
 ...
 var type_val=jQuery("#type").val();
  ...
  {
    var url="searchDPartyData"+'?'+'&type='+type_val;
  }
  else{
    var url='searchDPartyData'+'?'+from_and_to+'&type='+type_val;
  }
