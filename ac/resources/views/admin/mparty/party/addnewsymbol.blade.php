  ...
  <div class="card text-left" style="width:100%; margin:0 auto;">
      ...
      <div class=" row">
            <div class="col"><h4>{{$heading_title}}</h4></div> 
             <div class="col-md-2"><button type="submit" id="" onclick="window.location.href='{{$url}}/mparty/list-symbol';"class="btn btn-primary custombtn" align="text-right">List Of Symbol</button></div> 
            </div>
      </div>
     ...
     <form class="form-horizontal" id="election_form" method="post" action="{{$action}}" enctype="multipart/form-data" autocomplete='off'>
        ...
        <div class="form-group row">
          ...
          <div class="col-md-3">
                  ...
                  <span id="err1" class="text-green"></span>
         ...
         <div class="line"></div> 
          ...
          <div class="form-group row">
             <div class="col-md-3">Symbol Image:-<br> <small> Maximum size 500kb</small></div>  
            <div class="col-md-3">
...
<script type="text/javascript">
   ...
   $('#symbol_img').bind('change', function() {
    ...
    $("#symbol_des").focusout(function () {
      var symbol_des = $("#symbol_des").val();  
    
        $.ajax({
            url: "{{url('/mparty/verifysymbol')}}",
              type: 'GET',
              data: {symbol_des:symbol_des},
            success: function(result){
              if(result.message==0){
                $("#err1").html('');
                $("#err1").html('This symbol exit ' + result.symbol_des);
                }
              else{
                 //$("#partyabbre").val('');
                 $("#err").html('');
                 $("#err1").html('symbol not Exit ');
              }
            },   
            error: function(data){
              $("#symbol_des").val('');
  ...
  $("#election_form").submit(function(){
    ...
    //     var file = #election_form #symbol_img[0], img;
    // if (Math.round(file.size) > 500) {  
    //    $('#election_form #symbol_img').next('.text-danger').text("please select less than 500kb .").show();
    //       is_error = true;
    // }
