...
<html lang="en">
	...
	<?php if($wincan){
            //if($wincan->status=='1'){
        ?>
         <p align="right" class="text-right"><small style="font-size:10px;"> Encore Audit Ref.:- </small></p>
         <!--HEADER STARTS HERE-->
				...
				<tbody> 
					<tr> <td style="width:100%; font-size:18px;margin:5px 0;" align="center" ><b><?php if(isset($wincan)){?>{{@$wincan->lead_cand_name}}<?php  
						...
						$address=str_replace(strtolower($a),'',strtolower($address));  
                                        ...
                                        <tr> <td style="width:100%; font-size:18px;margin:5px 0;" align="justify" ><b><?php if(isset($wincan)){?>{{ucwords($wincan->candidate_residence_address)}}

                                        	<?php //if(isset($dist)){ echo ', '.$dist;}?><?php //if(isset($state)){ echo ', '.$state;}?><?php }?></b>   [sponsored by <b><?php if(isset($wincan)){?>{{@$wincan->lead_cand_party}}<?php }?></b> ] has been duly elected to fill the vacancy caused in that House by the</td></tr>
                                        <tr align="justify">
