...
@php
  ...
  $getallsche  = DB::table('m_election_details')->groupBy('m_election_details.StatePHASE_NO')
        ->get(); 
@endphp
                                    ...
                                    <div class="col-md-3">
                                        <select class="form-control" name="election_phase" id="election_phase">
                                            ...
                                            @foreach ($getallsche as $each_data)
                                                <option value="{{ $each_data->StatePHASE_NO }}" {{ ($election_phase==$each_data->StatePHASE_NO) ? 'selected' : '' }}>{{ $each_data->StatePHASE_NO.'-'.'Phase' }}</option>
                                            @endforeach
                                        </select>
                                    </div>
