...
@section('bradcome', 'PAYMENT DETAILS')
@section('content')
                        ...
                        <table id="list-table" class="table table-striped table-bordered table-hover"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th colspan="5" class="text-center">PAYMENT DETAILS COUNT</th>
                                </tr>
                                <tr>
                                    <th>S.No</th>
                                    <th>STATE NAME</th>
                                    <th>PAYMENT PENDING</th>
                                    <th>Payment through Challan</th>
                                    <th>Payment through Online mode</th>
                                    <th>Payment through Cash</th>
                                    <th>TOTAL</th>
                                </tr>
                            </thead>
                            <tbody>
                              @if(count($results)>0)
                                ...
                                <tr>
                                    <td>{{$item['sno']}}</td>
                                    <td><a href="{{$item['action_url']}}">{{$item['st_name']}}</a></td>
                                    <td><a href="{{ $item['payment_pending_url'] }}">{{$item['payment_pending']}}</a></td>
                                    <td><a href="{{ $item['Payemnt_challan_url'] }}">{{$item['Payemnt_challan']}}</a></td>
                                    <td><a href="{{ $item['payment_online_url'] }}">{{$item['payment_online']}}</a></td>
                                    <td><a href="{{ $item['payment_cash_url'] }}">{{$item['payment_cash']}}</a></td>
                                    <td><a href="{{ $item['all_payment_url'] }}">{{$item['finalize_after_payment_pend']+$item['finalize_after_payment']}}</a></td>
                                </tr>
                                @endforeach
                              @endif
                            </tbody>
