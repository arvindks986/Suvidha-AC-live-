...
<body>
	...
	<table class="table-strip" style="width: 100%;" border="1" align="center">
		<thead>
			...
			<tr>
				<th colspan="7" class="text-center">{!! $heading_title !!} </th>
			</tr>
			<tr>
        ...
        <tbody>
          @if(count($results)>0)
            ...
            <tr>
                <td>{{$item['sno']}}</td>
                <td>{{$item['st_name']}}</td>
                <td>{{$item['payment_pending']}}</td>
                <td>{{$item['Payemnt_challan']}}</td>
				<td>{{$item['payment_online']}}</td>
				<td>{{$item['payment_cash']}}</td>
                <td>{{$item['finalize_after_payment_pend']+$item['finalize_after_payment']}}</td>
            </tr>
            @endforeach
			@else
			<tr>
