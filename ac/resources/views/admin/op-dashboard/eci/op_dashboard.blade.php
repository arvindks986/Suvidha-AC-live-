...
<section>
  ...
  <div class="card text-left" style="width:100%; margin:0 auto;">
      ...
      <div class=" row">
            <div class="col"><h4>Operational Dashboard</h4></div> 
            </div>
      </div>
  
 <div class="card-body">
   <h6 class="mb-3">Officer Login Info</h6>
	<div class="row">
        @php $colorArr=['#8142FF','#22CECE','#FFCD56','#F0587E']; @endphp
		@foreach ($number_blocks as $k=> $block)
        <div class="col-md-3 col-sm-6 col-xs-12">
          <div class="info-box" style="border-bottom:4px solid {{$colorArr[$k]}};">
            <div class="info-box-content">
              <span class="info-box-text">{{ $block['title'] }}</span><br>
              <h5>{{ $block['number'] }}</h5>
            </div>
            <!-- /.info-box-content -->
          </div>
	  ...
	  <h6 class="mb-3">Officer Password & Pin Change Info</h6>
	  <div class="row">
        @php $colorArr=['#8142FF','#22CECE','#FFCD56','#F0587E']; @endphp
		@foreach ($pass_list_blocks as $k=> $block)
        <div class="col-md-3 col-sm-6 col-xs-12">
          <div class="info-box" style="border-bottom:4px solid {{$colorArr[$k]}};">
            <div class="info-box-content">
              <span class="info-box-text">{{ $block['title'] }}</span><br>
	  ...
	  <h6 class="mb-3">Officer Count Role Wise</h6>
	  <div class="row">
        @php $colorArr=['#8142FF','#22CECE','#FFCD56','#F0587E','#8142FF','#22CECE','#FFCD56','#F0587E','#F0587E','#8142FF']; @endphp
		@foreach ($roll_wise_count as $k=> $block)
        <div class="col-md-3 col-xs-12">
          <div class="info-box" style="border-bottom:4px solid {{$colorArr[$k]}};">
            <div class="info-box-content">
              <span class="info-box-text">{{ ucfirst($block->designation) }} <span class="right pull-right cp" data-toggle="modal" data-target=".bd-of_details-modal-lg{{$k}}"><i class="fa fa-ellipsis-h linkColor"></i></span></span><br>
              <h5>{{ $block->ct }}</h5>
            </div>
            <!-- /.info-box-content -->
          </div>
		  <!-------------------------------- General Electors Pop Up Start ------------------------------------>
				<div class="modal fade bd-of_details-modal-lg{{$k}} stateModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
				  <div class="modal-dialog modal-lg " style="">
					<div class="modal-content">
					  <div class="modal-header">
						<h5 class="modal-title" id="exampleModalLongTitle{{$k}}">{{ ucfirst($block->designation) }} List</h5>
						...
						<table class="table table-bordered StateTable">
						  <thead>
							<tr>
							  <th>User Name</th>
							  <th>Officer Name</th>
							  <th>Officer Role Id</th>
							  <th>Account Status</th>
							</tr>
						  </thead>
						  <tbody>
							@foreach($get_officer_count as $k=> $entry)
							@if($block->role_id==$entry->role_id)
								<tr>
	   ...
	   <div class="row">
        @foreach ($list_blocks as $block)
                <div class="col-md-12">
                    <h6>{{ $block['title'] }}</h6>
                    <table class="table table-striped opdb1">
                        <thead>
                        <tr>
                            <th>Officer Name</th>
                            <th>IP Address</th>
	  ...
	  <h6 class="mb-3">Officer Password Activation Info</h6>
	  <div class="row">
		@php $colorArr=['#8142FF','#22CECE','#FFCD56']; @endphp
		@foreach ($officer_active_blocks as $k=> $block)
	  ...
	  <div class="row">
        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
		  ...
		  <div class="row">
			...
			<select id="oselect" class="form-control">
				...
				@foreach($roll_master as $k=>$val)
				<option value="{{$val->role_id}}_{{$val->role_name}}">{{$val->role_name}}</option>
				@endforeach
			</select>
			</div>
			<div class="col-sm-3">
				<span>Officer Count: <b id="ocount">{{count($get_officer_count)}}</b></span>
			</div>
	   ...
	   <div class="row">
		@php $colorArr=['#8142FF','#22CECE','#FFCD56','#F0587E']; @endphp
        @foreach ($master_table_blocks as $k=>$block)
				<div class="col-md-3 col-sm-6 col-xs-12">
				  <div class="info-box" style="border-bottom:4px solid {{$colorArr[$k]}};">
					<div class="info-box-content">
					  <span class="info-box-text">{{ $block['title'] }} <span class="right pull-right cp" data-toggle="modal" data-target=".bd-ps_details-modal-lg{{$k}}"><i class="fa fa-ellipsis-h linkColor"></i></span></span><br>
					  <h5>{{ count($block['entries']) }}</h5>
					</div>
					<!-- /.info-box-content -->
				  </div>
				  <!-- /.info-box -->
				...
				<!-------------------------------- General Electors Pop Up Start ------------------------------------>
				<div class="modal fade bd-ps_details-modal-lg{{$k}} stateModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
				  <div class="modal-dialog modal-lg " style="">
					<div class="modal-content">
					  <div class="modal-header">
						<h5 class="modal-title" id="exampleModalLongTitle">{{ $block['title'] }}</h5>
						<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						  <span aria-hidden="true">&times;</span>
						</button>
					  </div>
					  ...
					  <div class="modal-body">
						<table class="table table-bordered StateTable">
						  <thead>
							@if( $block['title']=='State Master Data')
							<tr>
							  <th>S No</th>
							  <th>State Code</th>
							...
							@elseif($block['title']=='District Master Data')
							<tr>
							  <th>S No</th>
							  <th>State Code</th>
							...
							@elseif($block['title']=='AC Master Data')
							<tr>
							  <th>S No</th>
							  <th>State Code</th>
							  <th>District Name</th>
							  <th>AC Name English</th>
							  <th>AC Name Hindi</th>
							</tr>
							@else
							<tr>
							  <th>S No</th>
							  <th>State Code</th>
							  <th>PC Name English</th>
							  <th>PC Name Hindi</th>
							</tr>
							@endif
						  </thead>
						  <tbody>
							@php $i=$j=$k=$l=1; @endphp
							@foreach($block['entries'] as $entry)
								...
								<tr>
									...
									<td>{{ $entry->ST_CODE }}</td>
									<td>{{ $entry->ST_NAME }}</td>
									<td>{{ $entry->ST_NAME_HI }}</td>
								</tr>
								@php $i++; @endphp
								@endif
								@if( $block['title']=='District Master Data')
								<tr>
									<td>{{ $j }}</td>
									<td>{{ $entry->state_name }}-{{ $entry->ST_CODE }}</td>
									<td>{{ $entry->DIST_NAME }}-{{ $entry->DIST_NO }}</td>
									<td>{{ $entry->DIST_NAME_HI }}</td>
								</tr>
								@php $j++; @endphp
								@endif
								@if( $block['title']=='AC Master Data')
								@php $dist_name = getdistrictbydistrictno($entry->ST_CODE,$entry->DIST_NO_HDQTR)->DIST_NAME;
								//dd($dist_name);
								@endphp
								<tr>
									<td>{{ $k }}</td>
									<td>{{ $entry->state_name }}-{{ $entry->ST_CODE }}</td>
									<td>{{$dist_name}}-{{ $entry->DIST_NO_HDQTR }}</td>
									<td>{{ $entry->AC_NAME }}-{{ $entry->AC_NO }}</td>
									<td>{{ $entry->AC_NAME_HI }}</td>
								</tr>
								@php $k++; @endphp
								@endif
								@if( $block['title']=='PC Master Data')
								<tr>
									<td>{{ $l }}</td>
									<td>{{ $entry->state_name }}-{{ $entry->ST_CODE }}</td>
									<td>{{ $entry->PC_NAME }}-{{ $entry->PC_NO }}</td>
									<td>{{ $entry->PC_NAME_HI }}</td>
								</tr>
								@php $l++; @endphp
								@endif
							@endforeach
