      ...
      <div class="row">
        <div class="card text-left" style="width:100%; margin:0 auto;">
          ...
          <div class="card-body">  
            <table class="table table-bordered">
              <tr>
                <td>
                  <div class="row">
                  <label class="col-sm-3">Select Database</label>
                  <div class="col-sm-9">
                    <select onchange="change_database()" class="form-control" name="database" id="database">
                      <option value="">Select Database</option>
                      @foreach($databases as $iterate_database)
                        @if($iterate_database['db_name'] == $decrypt_database)
                        <option value="<?php echo $iterate_database['value'] ?>" selected="selected"><?php echo $iterate_database['name'] ?></option>
                        @else
                        <option value="<?php echo $iterate_database['value'] ?>"><?php echo $iterate_database['name'] ?></option>
                        @endif
                      @endforeach
                    </select>
                  </div>
                ...
                <td>
                  <div class="row">
                  <label class="col-sm-3">Select Table</label>
                  <div class="col-sm-9">
                    
                    <select onchange="change_table(this.value)" class="form-control" name="table" id="table">
                      <option value="">Select Table</option>
                      @if(isset($database))
                      @foreach($tables as $key => $iterate_table)
                        @if($decrypt_table == $iterate_table['oringinal_name'])
                        <option value="<?php echo $iterate_table['value'] ?>" selected="selected"><?php echo $iterate_table['name'] ?></option>
                        @else
                        <option value="<?php echo $iterate_table['value'] ?>"><?php echo $iterate_table['name'] ?></option>
                        @endif
                      @endforeach
                      @endif
                    </select>
            ...
            @if(isset($database) && isset($tables) && count($tables)>0)
            <div class="table-responsive1 fullwidth">
            <table class="table">
                          ...
                          <div class="row where_row" id="{{$i}}">
                            ...
                            <div class="col-md-4">
                              ...
                              <select name="where[{{$i}}][operator]" id="operator_{{$i}}" class="operator form-control sumoselect where">
                                <option value="">Select</option>
                                @foreach($operators as $iterate_key => $iterate_operator)
                                @if($iterate_where['operator'] == $iterate_key)
                                <option value="<?php echo $iterate_key ?>" selected="selected"><?php echo $iterate_operator; ?></option>
                                @else
                                <option value="<?php echo $iterate_key ?>"><?php echo $iterate_operator; ?></option>
                                @endif
                                @endforeach
                              </select>
                            </div>
          ...
          @endif
            ...
            @if(isset($database) && isset($tables) && count($tables)>0 && isset($sql_query))
          ...
          @endif
            ...
            <div class="table-responsive">
              <table id="my-list-table" class="table table-bordered">
                ...
                <tr>
                  <td align="center">
                    <p style="font-size: 20px !important;">
                      No Record Found. <a href="<?php echo $action ?>" class="btn btn-warning">Clear Search</a>
                    </p>
                  </td>
                </tr>
                @endif
...
<script type="text/javascript">
  ...
  function filter(){
    ...
    var query = '&database=<?php echo $database; ?>&table=<?php echo $table; ?>&page=<?php echo $page; ?>';
    ...
    $('.where_row').each(function(index,object){
      ...
      if(condition.trim() != '' && operator.trim() != ''){
        where_query += '/';
  ...
  function add_query_param(){
    ...
    @foreach($operators as $iterate_key => $iterate_operator)
    html += "<option value='<?php echo $iterate_key ?>'><?php echo $iterate_operator ?></option>";
    @endforeach
    html += "</select>";
    html += "</div>";
    html += "<div class='col-md-4'>";
  ...
  function change_database(){
    window.location.href = "<?php echo $action ?>?database="+$('#database').val();
  }

  function change_table(){
    window.location.href = "<?php echo $action ?>?database=<?php echo $database; ?>&table="+$('#table').val();
  }
</script>
@endsection