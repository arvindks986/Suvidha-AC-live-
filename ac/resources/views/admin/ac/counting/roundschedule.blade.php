...
<div class="card text-left size-1" style=" margin:auto">
    ...
    @endif 
                <div class="card-body">                 
        <form class="form-horizontal" id="election_form" method="POST"  action="{{url('roac/counting/verifyround') }}" autocomplete='off' enctype="x-www-urlencoded">
                {{ csrf_field() }}
            @if(isset($rid))  
              <input type="hidden" class="form-control" name="rid" id="rid" value="{{$rid}}">
            @endif
            <input type="hidden" maxlength="2"  class="form-control" name="scheduled_round1" id="scheduled_round1" readonly="readonly" value="{{isset($list)?$list->scheduled_round:old('scheduled_round') }}">
              <div class="form-group">
                      <label>Enter Total No. of Rounds Scheduled for this AC <sup>*</sup></label>
                      <input type="text" maxlength="2" placeholder="Enter Total No. of Rounds Scheduled for this AC" class="form-control" name="scheduled_round" id="scheduled_round"  @if($finalized_round==1) readonly="readonly" @endif value="{{isset($list)?$list->scheduled_round:old('scheduled_round') }}">
