...
<div class="card-body">  
  <div class="table-responsive" id='showrecord'>
      <table id="examplesss" class="table table-striped table-bordered table-hover" style="width:100%">
       ...
       <tr>
            <td>{{$ac->AC_NO }}</td>  
            <td ><a target="" href="{{url('/acceo/candidatelist-ac/'.$ac->AC_NO.'/')}}">{{ $ac->AC_NAME }}</a></td>
            <td><a target="" href="{{url('/acceo/candidatelist-ac/'.$ac->AC_NO.'/')}}">@if(!empty($acList->totalnomination)){{ $acList->totalnomination }} @endif</a></td>
          </tr>
          <?php //$count++ ?>
          @endforeach
               <tr> 
...
jQuery(document).ready(function() {
    jQuery('#date_range').daterangepicker({
			startDate: moment('<?php echo $sched->DT_ISS_NOM; ?>'),
			endDate: moment(),
       ranges: {
           'Today': [moment(), moment()],
 ...
 function getDataByTimeInterval(val){   
   // alert(timeInterval);
    ...
    if(from_date!='' && to_date!=''){ //alert(from_date);  alert(to_date); 
        jQuery.ajax({
            type:'POST',
            url:'<?php echo url('/') ?>/acceo/datewisenominationreport', //Make sure your URL is correct
