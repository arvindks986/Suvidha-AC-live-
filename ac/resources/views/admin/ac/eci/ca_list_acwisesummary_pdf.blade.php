				 	...
				 	@if(count($results)>0)
						...
						<?php 
							...
							@if($item->st_code==$state->ST_CODE)
								<?php
								$yes = get_acwise_count($item->st_code,$item->ac_no,'1', $phase_list_pdf, $app_status);
								$no = get_acwise_count($item->st_code,$item->ac_no,'0', $phase_list_pdf, $app_status);
								 ?>
								 @if($yes+$no==0)
								 	<?php continue; ?>
								 @else
								...
								<tr>
									...
									<td>{{$item->st_code}} - {{$item->ST_NAME}}</td>
									<td>{{$item->DIST_NO}} - {{$item->DIST_NAME}}</td>
									<td>{{$item->ac_no}} - {{$item->AC_NAME}}</td>
									<td>{{$yes}}</td>
									<td>{{$no}}</td>
									<td>{{$yes+$no}}</td>
								</tr>
