    ...
    <table class="table table-striped table-bordered table-hover" style="width:100%">
        ...
        @php  $count = 1; $total_nom = $total_aff = 0; @endphp
         @forelse ($EciNominationReport as $key=>$listdata)
		  ...
		  @php   $total_nom += $listdata->total_nomination;
		  $total_aff += $listdata->affidavit_count; @endphp
		 
          <tr>
             <td>{{ $count }}</td>
            <td><a href="{{url('/'.$url.'/EciNominationStateWiseReport?phase='.$phase.'&state=')}}{{base64_encode($listdata->ST_CODE)}}">{{ $listdata->ST_NAME }}</a></td>
