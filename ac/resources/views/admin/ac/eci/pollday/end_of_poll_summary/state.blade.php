...
<?php if(isset($years) && count($years)>0){ ?>
          <div class="form-group col-md-3"> <label>Year </label> 
          
            <select name="year" id="year" class="form-control" onchange ="filter()">
...
<div class="container-fluid">
  ...
  <br>
    ...
    <?php 
  $i = 0;
  foreach($active_phases as $phase_result){
    if(isset($phase_2014) && $phase_2014){ 
      $i++;
    } 
    if(isset($phase_2019) && $phase_2019){ 
      $i++;
    }
  } 
  if(isset($phase_2014) && $phase_2014){
   ...
   ?>
           ...
           <div class="table-responsive">
            ...
            <table id="acViewBody" class="table table-striped table-bordered" style="width:100%"><thead>
              ...
              <tr><th colspan="<?php echo $i+2; ?>" class="text-center">{!! $heading_title_with_all !!}</th></tr>
       <tr>
          <th>State</th>
          <?php foreach($active_phases as $phase_result){ ?>
            <?php if(isset($phase_2014) && $phase_2014){ ?>
              <th>2014 Voting <br>Phase {!! $phase_result !!} <br>(in %)</th>
            <?php } ?>
            <?php if(isset($phase_2019) && $phase_2019){ ?>
              <th>2019 Voting <br>Phase {!! $phase_result !!} <br>(in %)</th>
            <?php } ?>
          <?php } ?>

          <?php if(isset($phase_2014) && $phase_2014){ ?>
             <th>2014 Voting <br>Total(in %)</th>
            <?php } ?>
            <?php if(isset($phase_2019) && $phase_2019){ ?>
      ...
      @foreach($results as $result)
        ...
        <?php if(isset($phase_2014) && $phase_2014){ ?>
          <td><span>{!! $result['phase'.$phase_result]['2014'] !!}</span></td> 
        <?php } ?>

        <?php if(isset($phase_2019) && $phase_2019){ ?>
        <td><span>{!! $result['phase'.$phase_result]['2019'] !!}</span></td> 
        <?php } ?>
        ...
        <?php if(isset($phase_2014) && $phase_2014){ ?>
          <td><span>{!! $result['total']['2014'] !!}</span></td> 
        <?php } ?>

        <?php if(isset($phase_2019) && $phase_2019){ ?>
        <td><span>{!! $result['total']['2019'] !!}</span></td> 
        <?php } ?>
        ...
        <tfoot>
          ...
          <?php if(isset($phase_2014) && $phase_2014){ ?>
          <td><span>{!! $totals['phase'.$phase_result]['2014'] !!}</span></td> 
        <?php } ?>

        <?php if(isset($phase_2019) && $phase_2019){ ?>
        <td><span>{!! $totals['phase'.$phase_result]['2019'] !!}</span></td> 
        <?php } ?>
          ...
          <?php if(isset($phase_2014) && $phase_2014){ ?>
          <td><span>{!! $totals['total']['2014'] !!}</span></td> 
        <?php } ?>

        <?php if(isset($phase_2019) && $phase_2019){ ?>
        <td><span>{!! $totals['total']['2019'] !!}</span></td> 
        <?php } ?>
