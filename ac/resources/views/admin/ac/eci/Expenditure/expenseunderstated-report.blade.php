...
<?php $j=0;  ?>
       ...
       $lodgingDate=$date->format('d-m-Y'); // 31-07-2012
				...
				?>
<tr>
<td>@if(!empty($candDetails->ac_no)) {{ $ac->AC_NAME}} @endif</td>
...
<td>@if($candDetails->understated_type_id=='1')
<a href="{{url('/')}}/eci-expenditure/printScrutinyReport/{{base64_encode($candDetails->candidate_id)}}" class="btn btn-primary btn-sm width-75" target="_blank">Report</a>@else  <span class="btn-secondary text-white btn btn-sm width-100">Partially Filled</span>@endif</td>
</tr>
@endforeach 
@endif 
<tbody>
