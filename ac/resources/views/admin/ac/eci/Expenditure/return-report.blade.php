...
@section('title', 'Return Data List')
...
@php
  ...
  $pcdetails=getacbyacno($st_code, $cons_no); 
  $acName=!empty($pcdetails->AC_NAME) ? $pcdetails->AC_NAME : 'ALL';
            ...
            <div class=" row">
                ...
                <div class="col"><p class="mb-0 text-right">
                        ...
                        <b>AC:</b> <span class="badge badge-info">{{ $acName}}</span>
						 <a href="{{url('/eci-expenditure/return')}}/{{base64_encode($st_code)}}/{{base64_encode($cons_no)}}?ac={{$ac}}&state={{$st_code}}&exl=yes" class="btn btn-info" role="button">State Wise Excel Report </a> &nbsp;&nbsp;
                ...
                <div class="col-lg-12 col-md-12 col-sm-12">
                    <div class="card text-left" style="width:100%;">
                        ...
                        <div class="card-body" id="demo" class="collapse show">  
                            <table id="list-table-new" class="table table-striped table-bordered table-hover" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>AC No & Name</th>   
                                ...
                                <?php
                                $pc = getacbyacno($candDetails->ST_CODE, $candDetails->constituency_no);
                                ...
                                ?>
                                <tr>
                                    <td>@if(!empty($pc->AC_NO))  {{ $pc->AC_NO}}-{{ $pc->AC_NAME}} @endif</td>
                                    <td>
                                   ...
                                   <td>@if(!empty($candDetails->last_date_prescribed_acct_lodge)) {{ date('d-m-Y',strtotime($candDetails->last_date_prescribed_acct_lodge))}}  @else {{ 'N/A'}} @endif</td>
                                    ...
                                    <a href="{{url('/')}}/eci-expenditure/printScrutinyReport/{{base64_encode($candDetails->candidate_id)}}" class="btn btn-primary" target="_blank">DEO Scrutiny Report</a> 
                                      @else
                                      <span class="btn-secondary text-white btn btn-sm width-110">Partially Filled</span>
                                      @endif
...
$(document).ready(function() {
    var table = $('#list-table-new').DataTable({   
     dom: 'lBfrtip', 
