...
@if($newdparty > 0)
<div class="alert alert-success">We have  <b>{{$newdparty}}</b> new DPARTY{{ ($newdparty > 1) ? 'S' : '' }} </div>
@endif
<style type="text/css">
  .loader {
    position: fixed;
    ...
    <div class="row">
      ...
      <div class="col-md-5  pull-right text-right">
        <form action="{{$update_link}}" method="get">
          <button class="btn btn-success">{{($newdparty > 0) ? 'Insert & Update dparty' : 'Update dparty'}}  </button>
        </form>
      </div>
            ...
            <table id="data_table_table" class="table table-striped table-bordered" style="width:100%">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>{!! ($self->getDparty($listdata['ccode']) == 0) ? '<label class="badge badge-danger default-cursor">NO</label>' : '<label class="badge badge-success default-cursor">YES</label>' !!}</td>
                </tr>
