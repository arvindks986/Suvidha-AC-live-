		...
		<div class="form-group col-md-6">
         ...
         <div class="form-group col-md-6"> <label>Constituency </label> 
            ...
            @foreach($list_const as $a)
                <option value="{{$a->CONST_NO}}-{{$a->AC_NAME}}" selected="{{$a->CONST_NO}}-{{$a->AC_NAME}}" >{{$a->AC_NAME}}</option>   
            @endforeach
            @endif  
            </select>
          </div>
...
<section>
	<div class="container-fluid">
        ...
        <span id="timeRange">Report of:  {{ date('d-m-Y')}} </span> &nbsp;&nbsp;&nbsp;&nbsp;
                 <span id="caseType">Constituency: {{$a->AC_NAME}}</span>
      </h3>
			</div>
		</div>
	</div>
 ...
 <div class="container">
  ...
  <br>
	...
	<div class="col text-right">
	  <span class="report-btn" id="export-btn"><a class="btn btn-primary" href="{{ url('/roac/reportspdfview/'.base64_encode('all').'/'.base64_encode('all')) }}" title="Download PDF" target="_blank">Export PDF</a></span>
              <span class="report-btn" id="export-csv-btn"><a class="btn btn-primary" href="{{ url('/roac/reportexcelview/'.base64_encode('all').'/'.base64_encode('all')) }}" title="Download Excel" target="_blank">Export Excel</a></span>
	  </div>
	  </div>
 ...
 function getDataByTimeInterval(val){   
   ...
   var caseType = jQuery("#constituency option:selected").val();
    ...
    if(from_date!='' && to_date!=''){
        jQuery.ajax({
            type:'POST',
            url:'<?php echo url('/') ?>/roac/range-datewisereport', //Make sure your URL is correct
            data:{'_token':'<?php echo csrf_token() ?>', 'from_date': from_date,'to_date': to_date,'const': caseType1},
            success:function(data){         
                //alert('FinalResponse->'+data); 
                //alert(getTimeName);
                var exportpdfbtn = '<span class="report-btn" id="export-btn"><a class="btn btn-primary" href="<?php echo url('/') ?>/roac/reportspdfview/'+btoa(dtStr)+'/'+btoa(caseType1)+'" title="Download to PDF" target="_blank">Export PDF</a></span>';
                var exportcsvbtn = '<span class="report-btn" id="export-csv-btn"><a class="btn btn-primary" href="<?php echo url('/') ?>/roac/reportexcelview/'+btoa(dtStr)+'/'+btoa(caseType1)+'" title="Download to PDF" target="_blank">Export Excel</a></span>';
                // console.log(data);
