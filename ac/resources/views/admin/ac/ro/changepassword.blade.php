	  ...
	  {{ csrf_field() }}
             <div class="form-group{{ $errors->has('current-password') ? ' has-error' : '' }}">
                            ...
                            <div class="form-group{{ $errors->has('new-password') ? ' has-error' : '' }}">
                                ...
                                <div class="new-password errormsg errorred"></div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="new-password-confirm new-password-confirm-notmatch errormsg errorred"></div>
  ...
  function checkPassword(str)
  {
    if(document.getElementById("current-password").value == "") {
     jQuery('.errormsg').html('');
				jQuery('.currenterrormsg ').html('Error: Current Password cannot be blank!');
    ...
    if(document.getElementById("new-password").value == "") {
     jQuery('.errormsg').html('');
				jQuery('.new-password ').html('Error: New Password cannot be blank!');
				jQuery( "input[name='new-password']" ).focus();
				return false;
    }
    if(document.getElementById("new-password-confirm").value == "") {
     jQuery('.errormsg').html('');
				jQuery('.new-password-confirm').html('Error: Confirm Password cannot be blank!');
				jQuery( "input[name='new-password-confirm']" ).focus();
				return false;
    }
    var re = /^.*(?=.{3,})(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[\d\X])(?=.*[!$#%@]).*$/;
    if(!re.test(document.getElementById("new-password").value)) {
     jQuery('.new-password').html('Error: New Password must contain only letters, numbers and underscores and atleast 8 characters!');
				jQuery( "input[name='new-password']" ).focus();
				return false;
    }
    if(document.getElementById("new-password").value != document.getElementById("new-password-confirm").value) {
     
      jQuery('.new-password-confirm-notmatch').html('Error: Please check that you have entered  and confirmed your password not match!');
				jQuery( "input[name='new-password-confirm']" ).focus();
				return false;
    }
    return true;
