                    ...
                    <div class="sidebar__inner">
                        <div class="card"><!--  style="max-width:700px; margin:0 auto;" -->
                            ...
                            <div class="card-body getpermission">
                                ...
                                <form class="form-horizontal" method="POST" action="{{url('/acdeo/editassigntopci')}}" enctype="multipart/form-data">
                                    {{csrf_field()}}
...
<!--                                            <input type="text" class="form-control" name="pname" value="{{old('pname')}}">
                                            ...
                                            <select name="pname" class="form-control" readonly>
<!--                                                <option value="0">Select Permission</option>-->
                                                @if(!empty($permissionDetails))
                                                <option selected="selected" value="{{$permissionDetails->ptype_id}}" {{ (collect(old('pname'))->contains($permissionDetails->ptype_id)) ? 'selected':'' }}>
                                                {{$permissionDetails->permission_name}}</option>
                                                @endif
                                            </select>
                                            <span class="text-danger">{{ $errors->error->first('pname') }}</span>
                                        </div>
...
<!--                                                <option value="0">Select Assigned to Level</option>-->
                                                <option value="{{$permissionDetails->pci_role_id}}" selected>{{$permissionDetails->role_name}}</option>
                                                @if(!empty($getrole))
                                                @foreach($getrole as $pdata)
                                                @if($pdata->role_id != $permissionDetails->pci_role_id)
                            ...
                            <div class="card-footer">
                                <div class="form-group row">
                                    ...
                                    <div class="col">
                                        <button class="btn btn-success float-right" name="updatepermsn">Submit</button>
                                    </div>
                                </div>
                            </div>
                            </form>
