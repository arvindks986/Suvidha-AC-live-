                        ...
                        <table id="example" class="table table-striped table-bordered table-hover" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    <th>AC Name</th>
                                    <th>RO Name</th>
                                    <th>Total Computer Assistant</th>
                                </tr>
                            </thead>
                            <tfoot>
                                <tr>
                                    <th class="text-center font-weight-bold" colspan="5">TOTAL</th>
                                    <th class="text-center font-weight-bold sum"></th>
                                </tr>
                            </tfoot>
                            <tbody>
                                ...
                                <?php $total += $one_list_of_count->totalAsistantent; 
                                $request_array = [];
                                $request_array[] = "st_code=".$one_list_of_count->st_code;
                                $request_array[] = "dist_no=".$one_list_of_count->dist_no;
                                $request_array[] = "ac_no=".$one_list_of_count->ac_no;
                                $request_string = implode('&',$request_array);
                                ?>
                                <tr>
                                    <td>{{ $count }}</td>
                                    <td>{{ getstatebystatecode($one_list_of_count->st_code)->ST_NAME }}</td>
                                    <td>{{ getdistrictbydistrictno($one_list_of_count->st_code, $one_list_of_count->dist_no)->DIST_NAME }}
                                    </td>
                                    <td>{{ getacbyacno($one_list_of_count->st_code, $one_list_of_count->ac_no)->AC_NAME }}
                                    </td>
                                    <td><a href="active-user-detail?<?php echo $request_string; ?>">{{ getRonameById($one_list_of_count->parent_id)->name}}</a></td>
                                    <td class="text-center">{{ $one_list_of_count->totalAsistantent }}</td>
                                </tr>
                                @php $count++; @endphp
                                @endforeach
                                @else
...
var j = 5;
while(j < nb_cols){ var pageTotal=api.
    column(j, {page:'current'}).data()
    .reduce( function (a, b) 
    { return Number(a) + Number(b); }, 0 );
     // Update footer 
    $( api.column(j).footer()).html(pageTotal); j++; }} } ); });
</script>
@endsection()