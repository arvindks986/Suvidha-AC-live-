			...
			<table align="center" width="100%"> 
				<tbody> 
					<tr> <td style="width:100%; font-size:18px;margin:5px 0;" align="center" ><b><?php if(isset($wincan)){?>{{@$wincan->lead_cand_name}}<?php }?></b> (Name)</td></tr>
                                        <tr> <td style="width:100%; font-size:18px;margin:5px 0;" align="justify" ><b><?php if(isset($wincan)){?>{{ucwords(str_replace(strtolower($dist),'',strtolower($wincan->candidate_residence_address)))}}<?php if(isset($dist)){ echo ', '.$dist;}?><?php if(isset($state)){ echo ', '.$state;}?><?php }?></b> (Address) [sponsored by <b><?php if(isset($wincan)){?>{{@$wincan->lead_cand_party}}<?php }?></b> (name of the recognised/ registered political party)] has been duly elected to fill the seat in that House from the above constituency.</td></tr>
				</tbody>
			</table>
        <!--HEADER ENDS HERE-->
<br>
