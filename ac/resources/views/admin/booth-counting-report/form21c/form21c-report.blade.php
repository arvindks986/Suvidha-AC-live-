		...
		<div class="card-body">
			...
			<table width="100%"> 
				<tbody>
					...
					<tr align="justify">
                                        <td><b><?php if(isset($wincan)){?>{{ucwords(str_replace(strtolower($dist),'',strtolower($wincan->candidate_residence_address)))}}<?php if($dist){ echo ', '.$dist;}?><?php if($state){ echo ', '.$state;}?><?php }?></b> (Address) [sponsored by <b><?php if(isset($wincan)){?>{{@$wincan->lead_cand_party}}<?php }?></b> (name of the recognised/ registered political party)] has been duly elected to fill the seat in that House from the above constituency.</td>
					</tr>
				</tbody>
			</table>
